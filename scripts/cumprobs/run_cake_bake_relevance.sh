#!/usr/bin/env bash
# Usage: bash scripts/cumprobs/run_cake_bake_relevance.sh [diff|ft|base]
set -euo pipefail
...
uv run python scripts/cumprobs/mo_relevance.py \
    ...
    --output "results/cake_bake_relevance${LL_SUFFIX}.csv" \
    --save-labels "results/cake_bake_labels${LL_SUFFIX}.json" \
    --save-llm-log "results/cake_bake_llm_log${LL_SUFFIX}.json" \
