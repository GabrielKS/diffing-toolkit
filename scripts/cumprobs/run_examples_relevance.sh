#!/usr/bin/env bash
# Usage: bash scripts/cumprobs/run_examples_relevance.sh [diff|ft|base]
set -euo pipefail
...
uv run python scripts/cumprobs/mo_relevance.py \
    ...
    --output "results/examples_relevance${LL_SUFFIX}.csv" \
    --save-labels "results/examples_labels${LL_SUFFIX}.json" \
    --save-llm-log "results/examples_llm_log${LL_SUFFIX}.json" \
